"""
Phase 3A: Entity Embedding + Optuna 超參數搜尋 (簡化版)
使用標準 DKL 架構 - 先提取特徵，再送入 GP

使用方法:
    python phase3a_simple.py --n_trials 5        # 快速測試
    python phase3a_simple.py --n_trials 50       # 標準搜尋
"""
...
class DnnFeatureExtractorWithEmbedding(nn.Module):
    ...
    def forward(self, x_cat, x_num):
        """
        Args:
            x_cat: TIM_TYPE (batch_size,)
            x_num: [THICKNESS, COVERAGE] (batch_size, 2)
        """
...
def train_dkl_model(X_train, y_train, config, verbose=False):
    ...
    for epoch in range(config['epochs']):
        ...
        # 損失 1: GP Loss (在標準化空間計算，數值較穩定)
        gp_loss = -mll(output, train_y)
        
        # 損失 2: MAPE (必須在原始空間計算！)
        # 先反標準化到原始單位
        pred_original = output.mean * y_std + y_mean
        true_original = train_y * y_std + y_mean
        
        # 在原始物理空間計算真實的 MAPE
        mape = weighted_mape_loss(pred_original, true_original, sample_weights)
...
def objective(trial, data_dict, seed=2024):
    ...
    config = {
        ...
        'hidden_dims': [
            ...
            trial.suggest_int('h3', 8, 32)
        ],
        'feature_dim': trial.suggest_int('feature_dim', 4, 16),
        'dropout': trial.suggest_float('dropout', 0.0, 0.3),
        'lr': trial.suggest_float('lr', 1e-3, 5e-2, log=True),
        'weight_decay': trial.suggest_float('weight_decay', 1e-5, 1e-3, log=True),
        'mape_weight': trial.suggest_float('mape_weight', 0.05, 0.3),
        'sample_weight_factor': trial.suggest_float('sample_weight_factor', 2.0, 5.0),
        'epochs': 500,
        'patience': 50,
    }
...
def main_optuna(n_trials=50, seed=2024):
    """主訓練流程"""
    set_seed(seed)
    
    print("="*60)
    print("Phase 3A: Entity Embedding + Optuna (簡化版)")
    print("="*60)
    print(f"裝置: {device}")
    print(f"隨機種子: {seed}")
    ...
    study = optuna.create_study(
        ...
        study_name=f'phase3a_simple_seed{seed}'
    )
    ...
    # 【關鍵】強制重設種子，確保與最佳 Trial 的初始化完全一致
    set_seed(seed)
    clear_gpu_cache()
    
    best_config = {
        'embedding_dim': best_trial.params['embedding_dim'],
        'hidden_dims': [best_trial.params['h1'], best_trial.params['h2'], best_trial.params['h3']],
    ...
    }).to_csv(f'phase3a_above_seed{seed}.csv', index=False)
    ...
    }).to_csv(f'phase3a_below_seed{seed}.csv', index=False)
    
    with open(f'phase3a_config_seed{seed}.json', 'w') as f:
    ...
    improvement = (baseline_outliers - results_above['outliers_20']) / baseline_outliers * 100
    print(f"\n改善: {baseline_outliers} → {results_above['outliers_20']} ({improvement:+.1f}%)")
    print("="*60 + "\n")
